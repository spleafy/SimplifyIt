...
const FormField = ({
  ...
  return (
    <div className={`flex flex-col mb-5 ${className ? className : "mb-5"}`}>
      ...
      <div
        className={`rounded-md border-[1px] border-slate-200 px-4 placeholder:text-slate-200 transition-colors flex items-center dark:border-slate-500 ${
          loggedUser.settings
            ? "focus-within:border-theme-400 hover:border-theme-400"
            : "focus-within:border-primary-400 hover:border-primary-400"
        }`}
      >
        <input
          name={name}
        ...
        {validating ? (
          <svg
            role="status"
            className="w-6 h-6 animate-spin fill-primary-400"
            viewBox="0 0 100 101"
            fill="none"
