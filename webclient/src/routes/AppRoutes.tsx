...
import AccountDropdownPanel from "../components/AccountMenuDropdown";
...
const AppRoutes: FC = () => {
  ...
  const [workspacesShown, setWorkspacesShown] = useState(false);
  ...
  useEffect(() => {
    const effect = async () => {
      ...
      if (!response || response.status !== 200) {
        ...
        // Setting the user active workspace
          ...
          {loggedUser.settings.initialSetup ? (
            <div className="flex flex-col w-full">
                ...
                <div className="flex items-center w-full justify-end gap-5">
                  ...
                  >
                    ...
                    {notifications.length > 0 ? (
                      notifications.some(
                        (notification: NotificationType) => !notification.opened
                      ) ? (
                        <>
                  ...
                  <div
                    className="w-[35px] cursor-pointer relative tooltip tooltip-bl"
                    data-tooltip={activeWorkspace.name}
                    onClick={() => {
                      setWorkspacesShown(!workspacesShown);
                    }}
                  >
                    <ProfilePicture
                      color={activeWorkspace.settings.workspaceColor}
                  ...
                  <div
                    className={`w-[35px] cursor-pointer relative ${
                      !accountMenuShown ? "tooltip tooltip-bl" : ""
                    }`}
                    onClick={() => {
                      setAccountMenuShown(!accountMenuShown);
                    }}
                    data-tooltip={loggedUser.fullname}
                  >
                    <ProfilePicture
                    ...
                    {accountMenuShown ? (
                      <div className="flex absolute right-0 top-[50px]">
                        <AccountDropdownPanel setShown={setAccountMenuShown} />
                      </div>
                    ) : (
                      <></>
                    )}
              ...
              <div className="flex h-[calc(100%-55px)] w-full">
                ...
                <NotificationsPanel
                  setShown={setNotificationsShown}
                  shown={notificationsShown}
                />
                <WorkspacesPanel
                  setShown={setWorkspacesShown}
                  shown={workspacesShown}
                />
              </div>
              <SearchPanel setShown={setSearchShown} shown={searchShown} />
            </div>
          ) : (
            <></>
          )}
